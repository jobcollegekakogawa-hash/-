...
const convertTxtBtn = document.querySelector("#convertTxtBtn");
...
function normalizeDocLikeText(rawText) {
  return rawText
    .replace(/<[^>]+>/g, " ")
    .replace(/[_\r\n\t]+/g, " ")
    .replace(/\s+/g, " ")
...
function convertInputToTxtAndImport() {
  const normalizedText = normalizeDocLikeText(contractText.value);
  const hasContent = normalizedText.length > 0;
  contractText.value = normalizedText;

  if (!hasContent) {
    exportStatus.textContent = "入力欄が空のため、TXT変換できませんでした。契約書データを入力してください。";
    return;
  }

  const filename = `contract-input-${new Date().toISOString().replace(/[:.]/g, "-")}.txt`;
  triggerTextDownload(filename, normalizedText);
  exportStatus.textContent = `入力データをTXTに変換して取り込みました（${filename}）。`;
}
...
function getImportanceLabel(slide) {
  if (/(解除|解約|違反|期限|支払)/.test(slide.title + slide.body)) {
    return "重要度: 高";
  }

  if (/(金額|期間|更新|通知|秘密|個人情報)/.test(slide.title + slide.body)) {
    return "重要度: 中";
  }
...
function drawVideoSlide(slide, index, progress) {
  ...
  canvasContext.fillStyle = "#d47600";
  ...
  canvasContext.font = "700 24px system-ui, sans-serif";
  canvasContext.fillText(getImportanceLabel(slide), 930, 132);

  canvasContext.fillStyle = "#184067";
  ...
  canvasContext.fillStyle = "rgba(16, 53, 91, 0.88)";
  ...
  canvasContext.fillStyle = "rgba(23, 65, 100, 0.92)";
  canvasContext.font = "500 24px system-ui, sans-serif";
  const narrationLines = splitTextIntoLines(canvasContext, `音声ガイド: ${buildNarration(slide)}`, 980).slice(0, 2);
  narrationLines.forEach((line, lineIndex) => {
    canvasContext.fillText(line, 116, 600 + lineIndex * 32);
  });

  drawIllustration(index);

  canvasContext.fillStyle = "rgba(88, 132, 170, 0.28)";
  drawRoundedRect(canvasContext, 116, 624, 1048, 18, 9);
  canvasContext.fill();

  canvasContext.fillStyle = "#ff9f1a";
...
contractFile.addEventListener("change", async (event) => {
  ...
  const normalizedText = normalizeDocLikeText(rawContent);
  contractText.value = normalizedText;

  if (lowerName.endsWith(".doc") || lowerName.endsWith(".docx")) {
    const txtName = fileName.replace(/\.(doc|docx)$/i, ".txt");
    triggerTextDownload(txtName, normalizedText);
    exportStatus.textContent = `Wordファイルをテキスト化し、${txtName} をダウンロード可能にしました。`;
  }
});
